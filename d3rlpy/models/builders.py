...
def create_equivariant_continuous_q_function(
    ...
    if q_func_factory.share_encoder:  # in default setting share_encoder==False
        encoder = encoder_factory.create(
            observation_shape
        )
        # normalize gradient scale by ensemble size
    ...
    for _ in range(n_ensembles):
        ...
        # using equivariant procedure: equivariant_encoder --> invariant feature --> building non-constrained NN on top
        # Using num_hidden_layers==2 on the top block
        q_funcs.append(q_func_factory.create_equivariant_continuous(encoder))
...
# TODO: modify the equivariant version here
def create_equivariant_deterministic_policy(
    observation_shape: Sequence[int],
    action_size: int,
    encoder_factory: EncoderFactory,
) -> DeterministicPolicy:
...
# def create_equivariant_non_squashed_normal_policy(
#     observation_shape: Sequence[int],
#     action_size: int,
#     encoder_factory: EncoderFactory,
#     min_logstd: float = -20.0,
...
# ) -> NonSquashedNormalPolicy:
#     encoder = encoder_factory.create(observation_shape)
#     return EquivariantNonSquashedNormalPolicy(
#         encoder,
#         action_size,
#         min_logstd=min_logstd,
#         max_logstd=max_logstd,
...
def create_equivariant_value_function(
    ...
    encoder = encoder_factory.create(observation_shape)
    return EquivariantValueFunction(encoder)
